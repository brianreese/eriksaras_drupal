...
$(document).ready(function(){
	...
	$('#panelsWrap').attr('class', 'level1');
	var t;
	var r = false;
	var view = $(window);
	var isIE67 = false;

	// width variables, by breakpoint
	var $desk = 1000;
	...
	$('#panelsWrap').click(function(e){
		...
		if(!r){
			...
			if($(e.target).parents('#nav01').length || $(e.target).parents('#levelWrap_02').length){
					...
					if(_this.parents('#nav01').length){
						...
						switch(levelOpen){
							case 'level1':
								...
								levelOpen = 'level2';
								$('#panelsWrap').attr('class', 'level2');
								if(!_link.parents('li').hasClass('mobileNav')){
									_link.addClass('on');
									$('.mobileNav a').attr('href', _link.attr('href'));
								}
								else{
									$('#nav01 li:not(.mobileNav) a[href^="' + _link.attr('href') + '"').addClass('on');
								}
								
								if(view.width() > $desk){
									// console.log();
									$('#panelsWrap').stop().animate({ 'left':342 }, {duration: 400, easing: 'easeOutQuint', complete: function(){
									...
									$('#panelsWrap').stop().animate({ 'left':0 }, {duration: 400, easing: 'easeOutQuint', complete: function(){
										$('#'+elem).stop().fadeIn(200, function(){
											forceShow($(this));
											$('#wrap').height('auto');	
											// ---- setFixed ------
							...
							case 'level2':
								if(!_link.hasClass('on')){
									...
									var fn = function(){
										setAbsolute();
										if(view.width() > $desk){
											$('#panelsWrap').stop().animate({ 'left': 790 }, {duration: 400, easing: 'easeOutQuint', complete: function(){
												$('#'+elem).hide();	
											}});
										}
										else{
							...
							default:
								if(!_link.hasClass('on') && levelOpen == 'restore'){
									console.log('case1');
									Cufon.replace('#levelWrap_02 strong', {color: '#bcbcb9'});
									
									$('.article:visible, .nav02:visible').hide();
									$('#nav01 a, #levelWrap_02 a').removeClass('on');
									$('#wrap').height('auto');
									if(!_link.parents('li').hasClass('mobileNav')){
										_link.addClass('on');
									}
									...
									var fn = function(){
										...
										if(view.width() > $desk){
											$('.nav02:visible').find('.image').fadeOut(200, function(){
												$('#levelWrap_02').stop().animate({ 'margin-left':-191 }, {duration: 400, easing: 'easeOutQuint'});
												$('#panelsWrap').stop().animate({ 'left':191 }, {duration: 400, easing: 'easeOutQuint', complete: function(){
								...
								if(!_link.hasClass('on')){
									if(!_link.attr('href').match(/http/)){
										var fn = function(){
											...
											Cufon.replace('#levelWrap_02 strong');
											$('.article:visible').fadeOut(200, function(){
												$('#'+elem).fadeIn(200);
												// ---- setFixed ------
												setFixed();		
	...
	$('#cartPanelLink').click(function(e){
		e.preventDefault();
		rcp_cart_preview();
		// console.log(e);
		$('#wrap').toggleClass('cartView');
		$('.cartSummary').toggleClass('cartView');
		if(levelOpen == 'level1'){
			$('#wrap').addClass('level1open').removeClass('level2open');
		}
		else{
			$('#wrap').addClass('level2open').removeClass('level1open');	
		}
	});

	$('.node-add-to-cart').mousedown( function(e){
		// console.log(window);
		var i = 0;
	...
	function update_cart_count(i){
		setTimeout(function(){
			var count = $.getJSON('/cart/counter', function(result){
				console.log('loop ' + i);
				console.log(result);
	...
	Cufon.replace('#levelWrap_02 .cfn strong, #levelWrap_02 .arrow');
	...
	view.bind('resizeEnd', function() {
		if(levelOpen != 'level1'){
			...
			var xpos = $('#panelsWrap').offset().left;
			$('#nav01').css({
				'position':'fixed',
				'top': 'auto',
				'left': xpos
	...
	function setContentHeight(){
		...
		// if(windowHeight - 50 > 668){
		// 	windowHeight -= 102;
		// }
		if($('html').is('.ie6')){
			$('#rcp_content').css('height', windowHeight + 'px');	
		}
		else{
			$('#rcp_content').css('min-height', windowHeight + 'px');	
		}
	}
	...
	// var hhtoggle = function(){
	// 	$('img.hhelement').removeClass('showme');
	// 	$('img.hhelement').addClass('gohide');
	// 	setTimeout(
	// 		function(){
	// 			if (document.getElementById('panelsWrap').style.left === "788px"){	
	// 				$('img.hhelement').removeClass('showme');
	// 				$('img.hhelement').addClass('gohide');
	// 			}
	// 			else if (document.getElementById('panelsWrap').style.left === "790px"){	
	// 				$('img.hhelement').removeClass('showme');
	// 				$('img.hhelement').addClass('gohide');
	// 			}
	// 			else {
	// 				$('img.hhelement').removeClass('gohide');
	// 				$('img.hhelement').addClass('showme');
	// 			}
	// 		}
	// 	, 600);
	...
	function rcp_cart_preview() {
		$('#cartPreviewAjax').load('/cart', function(){
			// $(".cbklnk_container"+$nid+":first").attr('tabindex', '0').attr('style','background:none;outline:none;').remove(".cbklnk_container"+$nid+" .ajax-progress").focus();
		})
		
	};

	function rb_ajax_add($nid, $lang, $path_adjust) {
		jQuery(".cbklnk_container"+$nid).remove(".ajax-progress").append('<div class="ajax-progress ajax-progress-throbber"><div class="throbber">&nbsp;</div></div>').load($path_adjust+'/cbk/add/'+$nid+'/'+$lang, function(){
			jQuery(".cbklnk_container"+$nid+":first").attr('tabindex', '0').attr('style','background:none;outline:none;').remove(".cbklnk_container"+$nid+" .ajax-progress").focus();
		})
